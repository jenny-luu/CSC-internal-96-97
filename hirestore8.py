...
def check_input():
    ...
    if (entry_receipt.get().isdigit()):
        if int(entry_receipt.get()) < 0 or int(entry_receipt.get()) > 999999:
            Label(main_window, fg="red", text="Require six interger number or less", bg="sky blue") .grid(column=2, row=3,sticky =W)
            input_check = 1
    else:
        Label(main_window, fg="red", text="Require six interger number or less",bg="sky blue") .grid(column=2, row=3,sticky =W)
        input_check = 1
...
def setup_buttons():
    ...
    Label(main_window, text ="Type Items Or Select Items you want",
          font = "Times 10 bold", fg="red", bg="sky blue").grid(column = 5, row=1,sticky = E + S )
